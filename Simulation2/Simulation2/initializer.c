...
global.pinning_fmax = (double *)malloc(global.N_pinningsites*sizeof(double));
...
for(i=0;i<global.N_pinningsites;i++)
    ...
    while ((overlap==1)&&(N_trials<global.N_pinningsites*global.N_pinningsites))
        {
        ...
        for(j=0;j<i;j++)
            {
            //calculate distance
            dr = distance_folded_PBC(x_try, y_try, global.pinning_x[j], global.pinning_y[j]);
                if (dr < r_min)
                    {
                    overlap = 1; //found overlap
                    N_trials++;
        ...
        if (N_trials==global.N_pinningsites * global.N_pinningsites)
            {
            printf("Can't place pinning sites randomly, (system too dense) quitting\n");
            exit(1);
            }
        ...
        printf("Placed pinning site at %lf %lf R=%lf fmax=%lf\n",global.pinning_x[i],global.pinning_y[i],global.pinning_R[i], global.pinning_fmax[i]);
        }
