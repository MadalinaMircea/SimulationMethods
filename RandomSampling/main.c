...
double theta;
long num;
FILE *coords, *coords_inside, *dir_sam, *pi_m;
...
int main(int argc, const char * argv[]) {
    ...
    dir_sam= fopen("direct_samp_pi.txt", "a");
    //fprintf(dir_sam, "N          pi\n");

    //pi_m=fopen("pi_try.txt", "a");

    coords = fopen("coordinates_GR.txt","wt");
    coords_inside = fopen("coordinates_inside_BR.txt","wt");
    ...
    if((i == num) && num<N_total){
        fprintf(dir_sam,"%li %lf\n",num, 4.0 * N_inside/(double)N_thrown);
        num=num*18;
    }
    }
